...
if (isset($_POST['Register'])) {
    ...
    if (empty($my_option)) {
        ...
        $sql_e = "SELECT username FROM helper_data WHERE username='$username'";
            ...
            if (mysqli_stmt_execute($res_e)) {
                ...
                if (mysqli_stmt_num_rows($res_e) > 0) {
                    ...
                    if (empty($en_no_err) && empty($password_err)) {
                        $res_e = mysqli_prepare($connect, "INSERT INTO helper_data (username,email,password)VALUES ('$username','$email','$password')");
                        if (mysqli_stmt_execute($res_e)) {
