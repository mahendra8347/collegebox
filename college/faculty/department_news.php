<MARQUEE width=100% height=100% direction="up" scrolldelay="200" onmouseover="this.stop();" onmouseout="this.start();">
    ...
    if ($department == 'IT') {
        $run = mysqli_query($connect, "Select * from it_department_news order by 1 DESC");
    }elseif($department == 'CE'){
        $run = mysqli_query($connect, "Select * from ce_department_news order by 1 DESC");
    }
            ...
            <div>
                <table>
                    <tr>
                        ...
                        <td><a target="blank" href="more_news_detail.php?id=<?php echo $row["id"]; ?>" data-toggle="tooltip" title="Click For More Detail"><?php echo "<h4>$title</h4>" ?></a></td>
                        <td><?php echo $date; ?></td>
                    </tr>
                    <tr>
                        <td></td>
